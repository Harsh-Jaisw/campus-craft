    ...
    <div className="max-w-sm  w-full lg:max-w-[1024px] mx-auto lg:flex">
      <div className="gap-3 p-4 flex flex-col justify-between leading-normal">
        {item.details
          .filter(
            (data) =>
              data.programType.toLowerCase().includes(filterData.programType.toLowerCase()) 
              &&
              data.CourseDuration.toLowerCase().includes(filterData.courseDuration.toLowerCase()) &&
              data.CourseName.toLowerCase().includes(filterData.courseName.toLowerCase())
          )
          .map((data, index) => (
            <div key={index} className="border-2 rounded-md p-3 bg-[#E8E8E8]">
